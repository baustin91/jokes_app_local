...
$stmt = $mysqli->prepare("SELECT JokeID, Joke_question, Joke_answer, users.user_id, users.user_name, 
google_name FROM Jokes_table JOIN users ON users.user_id = Jokes_table.user_id WHERE Joke_question 
LIKE ?");
...
if ($stmt->num_rows > 0) {
  ...
  while ($stmt->fetch()) {
    ...
    echo "<div><p>" . $safe_joke_answer  . " -- Submitted by user " . $username . " " . 
    $google_name . "</p></div>";
  }
  echo "</div>";
} else {
